...
def test_span_creation_without_credentials(clean_env):
    ...
    with client.start_as_current_span(name="test-span") as span:
        ...
        if hasattr(span, "set_attributes"):
            span.set_attributes({"test.attribute": "test-value"})
...
try:
    ...
    if hasattr(client._resources, "tracer_provider"):
        ...
        if hasattr(provider, "_active_span_processor"):
            ...
            # Check if it's a LangfuseSpanProcessor
            from langfuse._client.span_processor import LangfuseSpanProcessor
            ...
            if hasattr(processor, "_span_processors"):
                ...
                print(f"   - Server export enabled: {processor.server_export_enabled}")
                assert not processor.server_export_enabled, (
                    "Server export should be disabled"
                )
                print("✅ PASS: Server export correctly disabled in Pixie-only mode")
        else:
...
try:
    with client.start_as_current_span(name="test-span") as span:
        ...
        print(f"   - Span ID: {span.span_id if hasattr(span, 'span_id') else 'N/A'}")
